...
interface OrbitImagesProps {
  ...
  className?: string;
  showPath?: boolean;
  // pathColor is now optional — when omitted, the path follows the
  // theme automatically (navy in light mode, white in dark mode)
  // via `pathColorClassName` below. Pass a literal value here only
  // if you want to override the theme-driven color.
  pathColor?: string;
...
function OrbitItem({ item, index, totalItems, path, itemSize, rotation, progress, fill, inverseScale }: OrbitItemProps) {
  ...
  return (
    ...
    >
      <div
        style={{
          transform: `rotate(${-rotation}deg) scale(${inverseScale})`,
...
export default function OrbitImages({
  ...
  showPath = false,
  pathColor,
  // Matches your existing connection-line theme pattern:
  // navy-300 in light mode, navy-600 in dark mode. Override via
  // the `pathColorClassName` prop if a section needs different tones.
  pathColorClassName = 'text-navy-300 dark:text-navy-600',
  pathWidth = 0,
  easing = 'linear',
  paused = false,
  centerContent,
  ...
  const containerHeight = responsive ? '60vh' : (typeof height === 'number' ? height : (typeof width === 'number' ? width : '50vh'));

  const inverseScale = responsive && scale ? 1 / scale : 1;
        ...
        >
          {showPath && (
            <svg
              ...
              className={`absolute inset-0 pointer-events-none ${pathColorClassName}`}
            >
              <path
                d={path}
                fill="none"
                // Explicit pathColor (if passed) always wins; otherwise
                // currentColor picks up whatever the Tailwind text-color
                // class above resolves to, so it tracks light/dark mode.
                stroke={pathColor ?? 'currentColor'}
